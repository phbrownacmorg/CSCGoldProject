...
if not excel_files:
    ...
    for excel_file in excel_files:
        ...
        temp = pd.read_excel(os.path.join(input_folder, excel_file), header=None, nrows=5) #This code block before dfTest is so that it can find where the actual header row is as the files don't have them in the same area
        ...
        excelFolder = os.path.join(working_dir, 'EmailListingProcessed')
        os.makedirs(excelFolder, exist_ok=True)

        #Move the excel file from the main folder to the EmailSentXLSX folder
        shutil.move(os.path.join(input_folder, excel_file), os.path.join(excelFolder, os.path.basename(excel_file)))
